...
function canBuildS1FromS2(s1, s2) {
    ...
    const s1Reduced = s1.toLowerCase().split(" ").join("")
    const s2Reduced = s2.toLowerCase().split(" ").join("")

    const obj1 = {}
    const obj2 = {}

    for(let x of s1Reduced){
        if(obj1[x] === undefined){
            obj1[x] = 1
        }else {
            obj1[x] += 1
    ...
    for(let x in obj1){
        if(obj2[x] === undefined || obj1[x] > obj2[x]){
            return false
        }
    }
