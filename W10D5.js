...
function amazonMusicRuntime(busDuration, songDurations) {
    var ride = busDuration - 30
    var waldo = ride
    var sorted = [...songDurations]
    sorted.sort(a,b => b,a)
    emptyArry = []
    for(let i = 0; i < songDurations.length;i++){
        waldo = ride - songDurations[i]

        if(sorted.indexOf(waldo) && sorted.indexOf(waldo) !== (-1)) {
            console.log("Found it")
            emptyArry.push([i, sorted.indexOf(waldo)])
        }
    }
